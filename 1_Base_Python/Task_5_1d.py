...
def main():
    ...
    device_parameter = input(f"Введите треубуемый параметр устройства"
                             f" {', '.join(london_co.get(device_name).keys())}: ").lower()
    print(london_co.get(device_name).get(device_parameter, "Такого параметра нет"))
